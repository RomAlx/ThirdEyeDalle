...
def telegram_bot(token):
    ...
    def callback_worker(call):
        ...
        elif call.data == "choose1":
            main_media_group[call.message.chat.id][3] = 1
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text=main_media_group[call.message.chat.id][0])
            bot.send_message(call.message.chat.id, "Секундочку, отправляю 1")
            generate_img(call.message)
        elif call.data == "choose2":
            main_media_group[call.message.chat.id][3] = 2
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text=main_media_group[call.message.chat.id][0])
            bot.send_message(call.message.chat.id, "Секундочку, отправляю 2")
            generate_img(call.message)
        elif call.data == "upgrade1":
            main_media_group[call.message.chat.id][3] = 3
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text=main_media_group[call.message.chat.id][0])
            bot.send_message(call.message.chat.id, "Секундочку, улучшаю 1")
            generate_img(call.message)
        elif call.data == "upgrade2":
            main_media_group[call.message.chat.id][3] = 4
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text=main_media_group[call.message.chat.id][0])
            bot.send_message(call.message.chat.id, "Секундучку, улучшаю 2")
            generate_img(call.message)
    ...
    def get_text_messages(message):
        ...
        if message.text == "Сгенерировать изображение":
            ...
            bot.send_message(message.from_user.id, "Я такой команды не знаю. Если нужна подсказка, то напишите /help.", reply_markup=main_menu)
    ...
    def generate_img(message):
        ...
        keyboard = types.InlineKeyboardMarkup()  # наша клавиатура
        key_choose1 = types.InlineKeyboardButton(text='Выбрать 1', callback_data='choose1')
        key_choose2 = types.InlineKeyboardButton(text='Выбрать 2', callback_data='choose2')
        key_upgrade1 = types.InlineKeyboardButton(text='Улучшить 1', callback_data='upgrade1')
        key_upgrade2 = types.InlineKeyboardButton(text='Улучшить 2', callback_data='upgrade2')
        keyboard.row(key_choose1, key_choose2)
        ...
        if main is None:
            ...
            bot.send_photo(chat_id=message.chat.id, photo=main[1], caption=main[0], reply_markup=main_menu)

        elif main[3] == 2:

            main_media_group[message.chat.id] = None
            bot.send_photo(chat_id=message.chat.id, photo=main[2], caption=main[0], reply_markup=main_menu)
