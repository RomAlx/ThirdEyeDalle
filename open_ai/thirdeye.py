...
async def generate_img(chat_id, message_id, prompt):
    try:
        #print('in gen')
        if main_media_group.get(chat_id) is None:
            ...
            global counter_all
            global start_time
            await main.bot.send_message(chat_id, WORK_MESSAGE)
            await main.bot.send_message(chat_id, WAIT_MESSAGE + str(counter_all//50+1) + ' мин')
            ...
            while len(queue_gen) > 0:
                #print('in while')
                if counter_all > 50 and (time.time_ns() - start_time) < 60:
                    #print('continue')
                    time.sleep(60-(time.time_ns()-start_time))
                    start_time = time.time_ns()
                #print('in work')
                counter_gen = counter_gen - 1
                ...
                await main.bot.delete_message(chat_id, (message_id + 1))
                await main.bot.delete_message(chat_id, (message_id + 2))
                await main.bot.send_media_group(gen[0], media=media)
                await main.bot.send_message(gen[0],
                                 text=f'👁 {gen[1]}\n\n[Третий Глаз](https://t.me/+BPwAeq0kYfxkZjMy)',
                                 parse_mode='MarkdownV2', disable_web_page_preview=True, reply_markup=SPECIAL_MENU)
    except Exception as ex:
        await main.bot.send_message(chat_id, EXCEPTION_MESSAGE)


async def upgrade_img(photo_num, chat_id, message_id):
    try:
        if photo_num == 1:
            await main.bot.send_message(chat_id, UPGRADE_MESSAGE1)
        if photo_num == 2:
            await main.bot.send_message(chat_id, UPGRADE_MESSAGE2)
        global counter_gen
        global counter_upd
        global counter_all
        global start_time
        ...
        while len(queue_upd)>0:
            ...
            await main.bot.delete_message(chat_id, (message_id + 1))
            await main.bot.delete_message(chat_id, (message_id + 2))
            await main.bot.send_photo(chat_id=upd[1], photo=image_url,
                           caption=f'👁 {prompt}\n\n[Третий Глаз](https://t.me/+BPwAeq0kYfxkZjMy)',
